...
/* --- إضافة KSU و SUSFS هنا --- */
#ifdef CONFIG_KSU
extern int ksu_handle_statmount(struct vfsmount *mnt);
#endif
					...
					int flag)
{
	...
	list_for_each_entry(r, &mnt->mnt_mounts, mnt_child) {
		...
		for (s = r; s; s = next_mnt(s, r)) {
			...
			/* ----- بداية خطاف تتبع المساحات المعزولة لـ SUSFS ----- */
#ifdef CONFIG_KSU_SUSFS
			susfs_copy_mount(p, q);
#endif
			/* ----- نهاية زرع SUSFS ----- */
		}
	}
...
static int do_add_mount(struct mount *newmnt, struct path *path, int mnt_flags)
{
	...
	/* --- خطاف التركيب الديناميكي لـ SUSFS و KSU --- */
	if (!err) {
#ifdef CONFIG_KSU
		ksu_handle_statmount(path->mnt);
#endif
#ifdef CONFIG_KSU_SUSFS
		susfs_handle_mount(newmnt, path);
#endif
	}
	/* --------------------------------------------- */
